<template>
  ...
  >
    ...
    >
      ...
      <div v-if="account.type === 'apikey'" class="space-y-4">
        <div>
          ...
          <input
            ...
            class="input"
            :placeholder="
              defaultBaseURLForPlatform(account.platform) ||
        ...
        >
          ...
          <template v-else>
            ...
            >
              <div class="mb-2 flex items-center justify-between gap-3">
                ...
                >
                  <Icon
                    ...
                    size="xs"
                    class="mr-1 inline"
                    :class="loadingVideoModelConfig ? 'animate-spin' : ''"
                  />
                  {{ t("common.refresh") }}
                </button>
              </div>
              ...
              >
                <option :value="0">
                  {{ t("admin.accounts.videoTemplateSelectPlaceholder") }}
                </option>
                <option
                  v-for="template in videoTemplateOptions"
                  :key="template.value"
            ...
            <div v-if="modelRestrictionMode === 'whitelist'">
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400">
                ...
                <span
                  ...
                  "
                  >{{ t("admin.accounts.supportsAllModels") }}</span
                >
              </p>
            </div>
            ...
            <div v-else>
              ...
              >
                <Icon
                  ...
                  size="sm"
                  class="mr-1.5 inline"
                  :class="fetchingVideoModels ? 'animate-spin' : ''"
                />
                {{
                  fetchingVideoModels
                    ? t("admin.accounts.syncUpstreamModelsLoading")
                    : t("admin.accounts.videoFetchUpstreamModels")
                }}
              </button>
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div class="min-w-0">
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.grokClientToolCache.hint') }}            </p>
          </div>
          <Toggle
        ...
        <template v-else>
          ...
          <div v-if="modelRestrictionMode === 'whitelist'">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400">
              ...
              <span
                v-if="allowedModels.length === 0 && modelMappings.length === 0"
                >{{ t("admin.accounts.supportsAllModels") }}</span
              >
            </p>
          </div>
      ...
      >
        <label class="input-label">{{
          t("admin.accounts.antigravityProjectIdLabel")
        }}</label>
        <input
      ...
      >
        ...
        >
          <div class="flex items-start gap-3 px-4 py-3">
            ...
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-center gap-2">
                <label class="input-label mb-0">{{
                  t("admin.accounts.openai.codexImageTool")
                }}</label>
                <span
          ...
          <div
            class="border-t border-sky-100 bg-white/70 p-2 dark:border-sky-900/50 dark:bg-dark-800/70"
          >
      ...
      >
        <div class="flex items-center justify-between gap-4">
          ...
          >
            <option value="x_api_key">
              {{ t("admin.accounts.anthropic.apiKeyAuthSchemeXApiKey") }}
            </option>
            <option value="authorization_bearer">
              {{ t("admin.accounts.anthropic.apiKeyAuthSchemeBearer") }}
            </option>
          </select>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            {{
              formatDateTime(
                new Date(String(account.extra.openai_compact_checked_at)),
              )
            }}
          </span>
        </div>
...
const baseUrlHint = computed(() => {
  ...
  if (props.account.platform === "video")
    return "视频上游服务根地址，例如 https://api.example.com；调用模板只配置路径。";
...
const getOpenAICompactModelMappingKey =
  createStableObjectKeyResolver<ModelMapping>(
    "edit-openai-compact-model-mapping",
  );
...
// OpenAI 订阅档位（Plus/Pro/Free）手动覆盖值，存于 credentials.plan_type；空值表示清空/自动识别
...
const webSearchGlobalEnabled = ref(false);const {
...
adminAPI.settings
  ...
  .then((cfg) => {
    webSearchGlobalEnabled.value =
      cfg?.enabled === true && (cfg?.providers?.length ?? 0) > 0;
  })
  .catch(() => {
...
>(() => [
  ...
  },
  {
    ...
    selectedCardClass:
      "border-amber-300 bg-amber-50 text-amber-900 shadow-sm ring-1 ring-amber-200 dark:border-amber-700 dark:bg-amber-900/25 dark:text-amber-100 dark:ring-amber-800",
  ...
  },
  {
    ...
    selectedCardClass:
      "border-rose-300 bg-rose-50 text-rose-900 shadow-sm ring-1 ring-rose-200 dark:border-rose-700 dark:bg-rose-900/25 dark:text-rose-100 dark:ring-rose-800",
...
// OpenAI 订阅档位手动覆盖选项（清空 + Plus/Pro/Free；保留别名或自定义值）
const planTypeOptions = computed(() =>
  buildPlanTypeOptions(
    editPlanType.value,
    t("admin.accounts.openai.planTypeClear"),
  ),
...
const openAIEndpointCapabilityOptions = computed<
  { value: OpenAIEndpointCapability; label: string }[]
>(() => [
...
const readOpenAIEndpointCapabilities = (
...
const defaultVideoTemplateID = computed(() => {
  const activeTemplate = videoTemplates.value.find(
...
const loadVideoModelConfig = async () => {
  ...
  videoModelConfigPromise = (async () => {
    ...
    const currentModel =
      modelRestrictionMode.value === "mapping"
        ? modelMappings.value.find((mapping) => isVideoExactModel(mapping.from))
            ?.from
        : allowedModels.value.find(isVideoExactModel);
    const existing = currentModel
      ? videoModels.value.find(
          (model) => model.public_model === currentModel.trim(),
        )
      : undefined;
    if (selectedVideoTemplateID.value <= 0 && existing?.template_id) {
      selectedVideoTemplateID.value = existing.template_id;
    }
  ...
  try {
    ...
    appStore.showError(
      t("admin.accounts.videoModelConfigLoadFailed", { message }),
    );
  } finally {
...
const collectVideoModelSyncItems = () => {
  const items = new Map<
    string,
    { publicModel: string; upstreamModel: string; templateID: number }
  >();
  const addItem = (
    publicModel: string,
    upstreamModel: string,
    ...
    if (
      !isVideoExactModel(normalizedPublicModel) ||
      !normalizedUpstreamModel ||
      normalizedUpstreamModel.includes("*")
    ) {
      return;
    }
    const templateID = Number(
      rawTemplateID ||
        selectedVideoTemplateID.value ||
        defaultVideoTemplateID.value,
    );
...
const ensureVideoModelsSynced = async () => {
  ...
  const existingByPublicModel = new Map(
    videoModels.value.map((model) => [model.public_model, model]),
  );
  for (const item of items) {
    if (item.templateID <= 0) {
      throw new Error(
        t("admin.accounts.videoTemplateRequiredForModel", {
          model: item.publicModel,
        }),
      );
    }
    const existing = existingByPublicModel.get(item.publicModel);
    const payload = buildVideoModelPayload(
      existing,
      item.publicModel,
      item.upstreamModel,
      item.templateID,
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  autoPause7dThreshold.value =
    typeof extra?.auto_pause_7d_threshold === "number"
      ? extra.auto_pause_7d_threshold * 100
      : null;
  ...
  if (
    ...
    openAICompactMode.value =
      (extra?.openai_compact_mode as OpenAICompactMode) || "auto";
    ...
    openaiOAuthResponsesWebSocketV2Mode.value = resolveOpenAIWSModeFromExtra(
      extra,
      {
    ...
    openaiAPIKeyResponsesWebSocketV2Mode.value = resolveOpenAIWSModeFromExtra(
      extra,
      {
    ...
    const credentials = newAccount.credentials as
      Record<string, unknown> | undefined;
    const compactMappings = credentials?.compact_model_mapping as
      Record<string, string> | undefined;
    if (compactMappings && typeof compactMappings === "object") {
      openAICompactModelMappings.value = Object.entries(compactMappings).map(
        ([from, to]) => ({ from, to }),
      );
    }
  }
  ...
  if (newAccount.platform === "antigravity") {
    ...
    const rawAgMapping = credentials?.model_mapping as
      Record<string, string> | undefined;
  ...
  if (
    ...
    headerOverrideEnabled.value =
      overrideCreds[HEADER_OVERRIDE_ENABLED_CREDENTIAL_KEY] === true;
  ...
  grokClientToolCacheEnabled.value =
    ...
    newAccount.type === "oauth" &&
    (grokClientToolCacheSetting === undefined ||
  ...
  if (newAccount.type === "apikey" && newAccount.credentials) {
    ...
    poolModeRetryStatusCodesInput.value = formatPoolModeRetryStatusCodes(
      bedrockCreds.pool_mode_retry_status_codes,
    );
    ...
    editVertexLocation.value =
      (credentials.location as string) ||
      (credentials.vertex_location as string) ||
      "us-central1";
    ...
    if (
      (newAccount.platform === "openai" || newAccount.platform === "grok") &&
      newAccount.credentials
    ) {
...
const syncAntigravityUpstreamModels = async () => {
  ...
  try {
    ...
    const upstreamModels = result.models
    ...
    for (const model of upstreamModels) {
      const exists = antigravityModelMappings.value.some(
        (mapping) => mapping.from === model,
      );
      if (!exists) {
    ...
    if (addedCount > 0) {
      ...
      appStore.showInfo(
        t("admin.accounts.syncUpstreamModelsNoChanges", {
    ...
    appStore.showError(
      t("admin.accounts.syncUpstreamModelsError", { message }),
    );
  } finally {
...
const fetchVideoUpstreamModels = async () => {
  ...
  try {
    ...
    for (const model of models) {
      const exists = modelMappings.value.some(
        (mapping) => mapping.from === model,
      );
      if (!exists) {
  ...
  try {
    ...
    if (props.account.type === "apikey") {
      ...
      if (poolModeEnabled.value) {
        ...
        const parsedRetryStatusCodes = parsePoolModeRetryStatusCodes(
          poolModeRetryStatusCodesInput.value,
        );
      ...
      if (isHeaderOverrideCapable(props.account.platform, 'apikey')) {
        if (headerOverrideEnabled.value) {
          ...
          if (headerError) {
            appStore.showError(
              t(`admin.accounts.headerOverride.${headerError}`),
            );
            return;
          }
        }
      ...
      const hasExistingServiceAccountJson = credentialsStatus
        ? Boolean(
            credentialsStatus.has_service_account_json ||
            credentialsStatus.has_service_account,
          )
        : Boolean(
    ...
    if (
      ...
      if (rpmLimitEnabled.value) {
        ...
        newExtra.base_rpm =
          baseRpm.value != null && baseRpm.value > 0
            ? baseRpm.value
            : DEFAULT_BASE_RPM;
    ...
    if (
      ...
      if (
        ...
        newExtra.openai_oauth_responses_websockets_v2_mode =
          openaiOAuthResponsesWebSocketV2Mode.value;
        newExtra.openai_oauth_responses_websockets_v2_enabled =
          isOpenAIWSModeEnabled(openaiOAuthResponsesWebSocketV2Mode.value);
      } else if (props.account.type === "apikey") {
        newExtra.openai_apikey_responses_websockets_v2_mode =
          openaiAPIKeyResponsesWebSocketV2Mode.value;
        newExtra.openai_apikey_responses_websockets_v2_enabled =
          isOpenAIWSModeEnabled(openaiAPIKeyResponsesWebSocketV2Mode.value);
      }
      ...
      if (isSparkShadow.value) {
        ...
        newExtra.openai_long_context_billing_enabled =
          openAILongContextBillingEnabled.value;
      }
      if (openAICompactMode.value === "auto") {
        delete newExtra.openai_compact_mode;      } else {
      ...
      if (props.account.type === "apikey") {
        ...
        newExtra.upstream_billing_probe_enabled =
          upstreamBillingAutoProbeEnabled.value;
      }
      if (
        autoPause5hThreshold.value != null &&
        autoPause5hThreshold.value > 0
      ) {
      ...
      switch (codexImageToolMode.value) {
        case "enabled":
        case "disabled":
          newExtra.codex_image_generation_bridge =
            codexImageToolMode.value === "enabled";
    ...
    if (props.account.type === "apikey" || props.account.type === "bedrock") {
      ...
      if (
        editDailyResetMode.value === "fixed" ||
        editWeeklyResetMode.value === "fixed"
      ) {
