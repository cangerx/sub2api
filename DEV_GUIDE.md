...
| 数据库凭据 | user=`ccapi`, password=`ccapi`, dbname=`ccapi` |
...
psql -U ccapi -h 127.0.0.1 -d ccapi -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER ccapi WITH PASSWORD 'ccapi';
...
```bash
# 连接数据库
psql -U ccapi -h 127.0.0.1 -d ccapi
...
# 执行 SQL 文件
psql -U ccapi -h 127.0.0.1 -d ccapi -f migration.sql
```
...
- [上游仓库](https://github.com/Wei-Shaw/ccapi)
