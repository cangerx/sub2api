...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/ccapi/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/ccapi"
DATA_DIR="/var/lib/ccapi/datamanagement"
SERVICE_FILE_NAME="ccapi-datamanagementd.service"
...
参数:
  ...
  --source  指定 ccapi 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/ccapi-src
EOF
}
...
if ! id ccapi >/dev/null 2>&1; then
  echo "[2/6] 创建系统用户 ccapi..."
  useradd --system --no-create-home --shell /usr/sbin/nologin ccapi
else
  echo "[2/6] 系统用户 ccapi 已存在，跳过创建"
fi
...
echo "[4/6] 准备数据目录..."
mkdir -p "$DATA_DIR"
chown -R ccapi:ccapi /var/lib/ccapi
chmod 0750 "$DATA_DIR"
...
systemctl enable --now ccapi-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status ccapi-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u ccapi-datamanagementd -f
2. 在 ccapi（容器部署时）挂载 socket:
