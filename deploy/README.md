# CCAPI Deployment Files

This directory contains files for deploying CCAPI on Linux servers and Apple-silicon Macs.
...
| `ccapi.service` | Systemd service unit file |
| `ccapi-datamanagementd.service` | datamanagementd systemd service unit file |
...
Apple-silicon Macs running macOS 26 can run the complete application, PostgreSQL, and Redis stack with Apple `container` 1.1.0 or newer:
...
The script uses Apple named volumes, starts dependencies in order, and performs live readiness checks. It does not provide a continuous restart supervisor; run `./apple-container.sh up` after a host reboot. Docker Compose remains the recommended production deployment path. The current Apple workflow uses an upstream-compatible Sub2API runtime image by default; consult the dedicated guide before overriding it with a custom image.
...
curl -sSL https://raw.githubusercontent.com/cangerx/sub2api/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs ccapi | grep "admin password"
   ```
...
# Restart CCAPI only
docker compose -f docker-compose.local.yml restart ccapi
...
| `ADMIN_EMAIL` | No | `admin@ccapi.local` | Admin email |
| `ADMIN_PASSWORD` | No | *(auto-generated)* | Admin password |
| `TZ` | No | `Asia/Shanghai` | Timezone |
| `UPDATE_GITHUB_REPO` | No | `cangerx/sub2api` | Repository used by the CCAPI online updater. |
| `UPDATE_GITHUB_TOKEN` | No | *(empty)* | Token for `api.github.com` release checks only; asset downloads remain anonymous. |
| `ENABLE_SERVER_TIMING` | No | `false` | Return `Server-Timing` for authenticated Admin UI requests. |
| `REDIS_USERNAME` | No | *(empty)* | Redis ACL username; leave empty for the bundled Redis service/default user. |
| `GATEWAY_IMAGE_NONSTREAM_KEEPALIVE_INTERVAL` | No | `0` | Non-streaming image JSON keepalive interval in seconds; `0` disables it. |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf ccapi-complete.tar.gz deployment/

# Transfer to new server
scp ccapi-complete.tar.gz user@new-server:/path/to/destination/
...
CCAPI supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/cangerx/sub2api/releases)
2. Extract and copy the binary to `/opt/ccapi/`
3. Copy `ccapi.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/ccapi.service
   ```
...
The main config file is at `/etc/ccapi/config.yaml` (created by Setup Wizard).
...
/opt/ccapi/
├── ccapi              # Main binary
├── ccapi.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 ccapi
...
# View detailed logs
docker compose logs --tail=100 ccapi
...
# View recent logs
sudo journalctl -u ccapi -n 50

# Check config file
sudo cat /etc/ccapi/config.yaml
...
CCAPI supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.ccapi.org](https://tls.ccapi.org/)** to get TLS fingerprint information for different devices and browsers.
