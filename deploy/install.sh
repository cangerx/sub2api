...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="CCAPI 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 CCAPI 服务："
    ...
    ["not_installed"]="CCAPI 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 CCAPI。"
    ...
    ["uninstall_complete"]="CCAPI 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 CCAPI"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 CCAPI"
    ...
    ["server_config_desc"]="配置 CCAPI 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="CCAPI Installation Script"
    ...
    # Completion
    ["install_complete"]="CCAPI installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start CCAPI service:"
    ...
    ["not_installed"]="CCAPI is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove CCAPI from your system."
    ...
    ["uninstall_complete"]="CCAPI has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install CCAPI"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove CCAPI"
    ...
    ["server_config_desc"]="Configure CCAPI server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/ccapi" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/ccapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="ccapi_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/ccapi" "$INSTALL_DIR/ccapi"
    chmod +x "$INSTALL_DIR/ccapi"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/ccapi"
}
...
User=ccapi
Group=ccapi
WorkingDirectory=/opt/ccapi
ExecStart=/opt/ccapi/ccapi
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=ccapi
...
enable_autostart() {
    ...
    if systemctl enable ccapi 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status ccapi"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u ccapi -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart ccapi"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop ccapi"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/ccapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet ccapi; then
        print_info "$(msg 'stopping_service')"
        systemctl stop ccapi
    fi

    # Backup current binary
    cp "$INSTALL_DIR/ccapi" "$INSTALL_DIR/ccapi.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/ccapi.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/ccapi"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start ccapi

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: CCAPI must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/ccapi" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="ccapi.backup.${current_version}"
        else
            backup_name="ccapi.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/ccapi" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop ccapi 2>/dev/null || true
    systemctl disable ccapi 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/ccapi.service
    systemctl daemon-reload
