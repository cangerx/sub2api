...
func (c *concurrencyCache) AcquireVideoUserSlot(ctx context.Context, userID int64, maxConcurrency int, taskID string) (bool, error) {
	result, _, err := runScriptInt64Pair(ctx, c.rdb, acquireScript, []string{videoUserSlotKey(userID)}, maxConcurrency, c.slotTTLSeconds, taskID)
	if err != nil {
		return false, err
	}
	return result == 1, nil
...
func (c *concurrencyCache) AcquireVideoAPIKeySlot(ctx context.Context, apiKeyID int64, maxConcurrency int, taskID string) (bool, error) {
	result, _, err := runScriptInt64Pair(ctx, c.rdb, acquireScript, []string{videoAPIKeySlotKey(apiKeyID)}, maxConcurrency, c.slotTTLSeconds, taskID)
	if err != nil {
		return false, err
	}
	return result == 1, nil
...
func (c *concurrencyCache) AcquireVideoAccountSlot(ctx context.Context, accountID int64, maxConcurrency int, taskID string) (bool, error) {
	result, _, err := runScriptInt64Pair(ctx, c.rdb, acquireScript, []string{videoAccountSlotKey(accountID)}, maxConcurrency, c.slotTTLSeconds, taskID)
	if err != nil {
		return false, err
	}
	return result == 1, nil
...
func (c *concurrencyCache) CleanupStaleProcessSlots(ctx context.Context, activeRequestPrefix string) error {
	...
	// 视频异步任务使用独立槽位 key，未纳入同步请求活跃索引；启动时仅扫描 video:* 前缀清理死进程残留。
	for _, pattern := range []string{videoAccountSlotKeyPrefix + "*", videoUserSlotKeyPrefix + "*", videoAPIKeySlotKeyPrefix + "*"} {
...
// cleanupSlotsByPattern 扫描匹配 pattern 的有序集合键，逐 key 清理非当前进程成员，兼容 Redis Cluster 单 key Lua 限制。
func (c *concurrencyCache) cleanupSlotsByPattern(ctx context.Context, pattern, activePrefix string) error {
	...
	for {
		...
		for _, key := range keys {
			if _, _, err := runScriptInt64Pair(ctx, c.rdb, startupCleanupSlotScript, []string{key}, activePrefix, c.slotTTLSeconds); err != nil {
