...
func RegisterGatewayRoutes(
	...
	videoScoped := func(next gin.HandlerFunc) gin.HandlerFunc {
	...
	videoStatusDispatchHandler := func(c *gin.Context) {
		if getGroupPlatform(c) == service.PlatformGrok {
			h.OpenAIGateway.GrokVideoStatus(c)
			return
		}
		videoScoped(h.Video.Get)(c)
	}
	...
	videoContentDispatchHandler := func(c *gin.Context) {
		if getGroupPlatform(c) == service.PlatformGrok {
			c.Params = append(c.Params, gin.Param{Key: "request_id", Value: c.Param("id")})
			h.OpenAIGateway.GrokVideoContent(c)
			return
		}
		videoScoped(h.Video.Content)(c)
	}
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		gateway.POST("/videos", videoScoped(h.Video.Create))
		gateway.GET("/videos", videoScoped(h.Video.List))
		gateway.GET("/videos/models", videoScoped(h.Video.Models))
		gateway.POST("/videos/generations", videoGenerationHandler)
		gateway.POST("/videos/edits", videoEditHandler)
		gateway.POST("/videos/extensions", videoExtensionHandler)
		gateway.GET("/videos/:id/content", videoContentDispatchHandler)
		gateway.GET("/videos/:id/content.mp4", videoScoped(h.Video.Content))
		gateway.POST("/videos/:id/cancel", videoScoped(h.Video.Cancel))
		gateway.GET("/videos/:id", videoStatusDispatchHandler)
	}
