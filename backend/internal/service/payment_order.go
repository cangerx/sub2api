...
import (
	...
	"github.com/Wei-Shaw/ccapi/internal/pkg/servertiming"
...
func calculateCreateOrderPayAmountForOrder(orderType string, amount, feeRate, balanceMultiplier float64, currency string) (string, float64, error) {
	paymentAmount := amount
	if orderType == payment.OrderTypeSubscription {
		paymentAmount = calculateGatewayPaymentAmount(amount, balanceMultiplier, currency)
	}
	return calculateCreateOrderPayAmount(paymentAmount, feeRate, currency)
}
