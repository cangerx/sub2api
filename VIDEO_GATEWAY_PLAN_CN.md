...
本文档规划 CCAPI 的视频模型异步调用能力。目标是对用户提供统一的 OpenAI 风格视频接口，对内通过「调用模板 + 视频模型目录」适配不同上游的视频任务 API，并支持按次、按秒、按区间计费。
...
用户、前端、第三方客户端统一使用 CCAPI 的 OpenAI 风格视频协议，不直接暴露各家上游的原始调用方式。

```text
用户请求 -> CCAPI 统一视频协议 -> video_model -> 调用模板 -> 上游真实接口（账号/代理走 Account）
```
