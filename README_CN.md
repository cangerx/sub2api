...
- **服务条款风险**：使用本项目可能违反 Anthropic 等上游服务商的服务条款。请在使用前仔细阅读相关服务商的用户协议。
- **合规使用**：请在符合您所在国家或地区法律法规的前提下使用本项目，严禁用于任何违法违规用途。
- **免责声明**：本项目仅供技术学习与研究使用，作者不对账户封禁、服务中断、数据丢失或其他损失承担责任。

## 项目概述

CCAPI 是面向团队与商业化场景的 AI 网关平台，用于把多账号、多订阅、多上游渠道统一封装为稳定的 OpenAI 兼容 API 服务。

平台提供账号池、密钥分发、模型路由、用量计费、支付充值、配额观测、媒体记录与运营监控等完整能力。用户只需要使用平台生成的 API Key，CCAPI 负责鉴权、调度、故障转移、用量记录与请求转发。

## 核心功能

| 模块 | 能力 |
|------|------|
| 网关 | OpenAI 兼容路由、流式传输、粘性会话、故障转移、模型级策略 |
| 账号 | 多上游账号池，支持 OAuth 与 API Key 等账号形态 |
| 模型 | 文本、代码、图片、视频、Grok/xAI、Antigravity、Claude、Gemini、OpenAI 兼容渠道 |
| 计费 | Token 级用量记录、成本计算、余额控制、防透支 |
| 支付 | 内置 EasyPay、支付宝、微信支付、Stripe、订阅订单和充值汇率 |
| 运营 | 管理后台、趋势监控、使用记录、媒体预览、账号健康检查 |
| 安全 | 速率限制、并发限制、URL 白名单、响应头过滤、合规确认 |
| 扩展 | 外部系统 iframe、分组策略、模型别名、自定义工具 schema |
...
<td>感谢 AIGoCode 赞助了本项目！AIGoCode 是一站式集成 Claude Code、Codex 以及最新 Gemini 模型的综合平台，为您提供稳定、高效、高性价比的 AI 编程服务。平台提供灵活的订阅方案，零封号风险，免 VPN 直连，响应极速。AIGoCode 为 CCAPI 用户准备了专属福利：通过<a href="https://aigocode.com/invite/SUB2API">此链接</a>注册，首次充值可额外获得 10% 赠送额度！</td>
</tr>
...
<td>感谢 丝绸API 赞助了本项目！ <a href="https://code.silkapi.com/register?aff=SUB2API">丝绸API</a> 是基于 CCAPI 搭建的中转服务，专注于提供 Codex 高速稳定API中转。</td>
</tr>
...
<td>感谢 AICodeMirror 赞助了本项目！AICodeMirror 提供 Claude Code / Codex / Gemini CLI 官方高稳定性中转服务，企业级并发、快速开票、7×24 小时专属技术支持。Claude Code / Codex / Gemini 官方通道低至原价 38% / 2% / 9%，充值更享额外折扣！AICodeMirror 为 CCAPI 用户提供专属福利：通过<a href="https://www.aicodemirror.com/register?invitecode=KMVZQM">此链接</a>注册，首次充值立享 8 折优惠，企业客户最高可享 75 折！</td>
</tr>
...
<td>感谢 RoxyBrowser 赞助本项目！<a href="https://roxybrowser.com/invite/bgGKG7">RoxyBrowser</a> 是 CCAPI 的理想搭档：内置原生 Roxy AI Agent 与高质量原生住宅 IP，支持通过简单命令实现批量自动化，显著提升多账号管理的安全性与效率！点击<a href="https://roxybrowser.com/invite/bgGKG7">此链接</a>注册，可领取免费住宅 IP 套餐与终身 9 折优惠。
</td>
</tr>

<tr>
<td width="180"><a href="https://apikl.com"><img src="assets/partners/logos/apikl.png" alt="apikl" width="150"></a></td>
<td>感谢 Apikl 赞助本项目！平台基于 CCAPI 搭建，为开发者提供 Codex / Claude 系列模型的中转服务，专注于长期稳定、高速直连与高性价比。支持按量计费的余额结算、企业级正规发票及一对一专属对接。<a href="https://apikl.com">立即注册</a>即享充值 1:1 赠送 — 余额翻倍！
</td>
</tr>
...
| 项目 | 说明 | 功能 |
|------|------|------|
| ~~[Sub2ApiPay](https://github.com/touwaeriol/sub2apipay)~~ | ~~自助支付系统~~ | **已内置** — 支付功能已集成到 CCAPI 中，无需独立部署。详见 [支付配置指南](docs/PAYMENT_CN.md) |
...
通过 Nginx 反向代理 CCAPI（或 CRS 服务）并搭配 Codex CLI 使用时，需要在 Nginx 配置的 `http` 块中添加：
...
部署脚本会下载生产 Compose 模板、生成安全密钥、创建本地数据目录，并准备 `.env` 文件。
...
适合已经有 PostgreSQL 和 Redis 的服务器，或希望使用 systemd 管理服务的部署方式。

```bash
curl -sSL https://raw.githubusercontent.com/cangerx/sub2api/main/deploy/install.sh | sudo bash
```

安装脚本会从 GitHub Release 下载最新版本，安装到 `/opt/ccapi`，创建 `ccapi` systemd 服务，并在 `8080` 端口打开首次配置向导。
...
适合你已经维护独立 PostgreSQL 和 Redis 的环境。
...
CCAPI 支持 [Antigravity](https://antigravity.so/) 账户，授权后可通过专用端点访问 Claude 和 Gemini 模型。
